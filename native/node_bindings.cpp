...
Value LaunchKernel(const CallbackInfo &info)
{
    ...
    {
        throw TypeError::New(env, "Kernel ID, args, grid dims, and block dims expected");
    }
    ...
    try
    {
        ...
        for (uint32_t i = 0; i < args.Length(); i++)
        {
            ...
            if (arg.IsObject())
            {
                ...
                if (obj.Has("id") && obj.Has("size"))
                {
                    ...
                    {
                        void *ptr = bufIt->second->getPointer();
        ...
        // Get dimensions
        dim3 grid(
            gridDims.Get((uint32_t)0).As<Number>().Uint32Value(),
            gridDims.Length() > 1 ? gridDims.Get((uint32_t)1).As<Number>().Uint32Value() : 1,
            gridDims.Length() > 2 ? gridDims.Get((uint32_t)2).As<Number>().Uint32Value() : 1);

        dim3 block(
            blockDims.Get((uint32_t)0).As<Number>().Uint32Value(),
            blockDims.Length() > 1 ? blockDims.Get((uint32_t)1).As<Number>().Uint32Value() : 1,
            blockDims.Length() > 2 ? blockDims.Get((uint32_t)2).As<Number>().Uint32Value() : 1);
