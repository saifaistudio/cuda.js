...
#define CU_CHECK(call)                                                   \
    ...
    {
        nvrtcProgram prog;
        NVRTC_CHECK(nvrtcCreateProgram(&prog, source.c_str(), "kernel.cu", 0, nullptr, nullptr));

        const char *opts[] = {"--gpu-architecture=compute_50", "--fmad=false"};
        ...
        if (logSize > 1)
        {
            ...
            if (compileResult != NVRTC_SUCCESS)
            {
                ...
                throw std::runtime_error(std::string("Kernel compilation failed: ") + log.data());
            }
        }
    ...
    void CudaKernel::launch(void **args, dim3 gridDim, dim3 blockDim, size_t sharedMem, cudaStream_t stream)
    {
